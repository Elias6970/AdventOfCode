...
input1="""turn on 0,9 through 959,629
toggle 717,493 through 930,875
"""

matriz=numpy.zeros((999,999),dtype=bool)
#primer número de la matriz es para bajo y el segundo para el lado
input=input.split("\n")
input_info=[]

for i in input:
    input_splitted=i.split(" ")
    if len(input_splitted)==5:
        input_info=[input_splitted[1],input_splitted[2],input_splitted[4]]
    elif len(input_splitted)<5:
        input_info=[input_splitted[0],input_splitted[1],input_splitted[3]]

    start_range=input_info[1].split(",")
    end_range=input_info[2].split(",")

    start_range_x=int(start_range[0])
    start_range_y=int(start_range[1])
    end_range_x=int(end_range[0])
    end_range_y=int(end_range[1])
    
    status=input_info[0]

    count=1
    actual_position_x=start_range_x
    actual_position_y=start_range_y
    if status=="on":
        while actual_position_y!=end_range_y:
            while actual_position_x!=end_range_x: #cambia x
                matriz[actual_position_x,start_range_y]=True
                actual_position_x=start_range_x+count
                count=count+1
                
                #print(count,matriz[actual_position,start_range_y])
            actual_position_y=actual_position_y+1
            actual_position_x=start_range_x
            count=1
    
    if status=="off":
        while actual_position_y!=end_range_y:
            while actual_position_x!=end_range_x: #cambia y
                matriz[actual_position_x,start_range_y]=False
                actual_position_x=start_range_x+count
                count=count+1
    ...
    if status=="toggle":
        while actual_position_y!=end_range_y:
            while actual_position_x!=end_range_x: #cambia x
                if matriz[actual_position_x,start_range_y]==False:
                    matriz[actual_position_x,start_range_y]=True
                else:
                    matriz[actual_position_x,start_range_y]=False
                actual_position_x=start_range_x+count
...
valid=0
noValid=0
for e in numpy.nditer(matriz,order="F"):
    if e==True:
        valid=valid+1
    elif e==False:
        noValid=noValid+1
